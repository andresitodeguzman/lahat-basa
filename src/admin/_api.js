// API URLS
let adminGetAll = '/api/Admin/getAll.php';
let adminAdd = '/api/Admin/add.php';
let adminDelete = '/api/Admin/delete.php';

let customerGetAll = '/api/Customer/getAll.php';

let employeeGetAll = '/api/Employee/getAll.php';
let employeeAdd = '/api/Employee/add.php';
let employeeDelete = '/api/Employee/delete.php';

let transactionGetAll = '/api/Transaction/getAll.php';

let productGetAllApi = '/api/Product/getAll.php';
let productDelete = '/api/Product/delete.php';
let productAdd = '/api/Product/add.php';
let productUpdateCategoryId = '/api/Product/updateCategoryId.php';

let transitemGetByProductIdApi = '/api/TransItem/getByProductId.php';

let categoryGetAll = '/api/Category/getAll.php';
let categoryUpdate = '/api/Category/update.php';
let categoryDelete = '/api/Category/delete.php';
let categoryAdd = '/api/Category/add.php';